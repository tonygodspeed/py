...
echo `date +%H:%M:%S`
mkdir $an_path
tar zcvf $an_path/$an_date.regular.tar.gz $raw_path/
if [ ! $1 ]; then		
	echo 'done' > $an_path
fi

hadoop fs -rm -r /user/client/analyze_data/$ten_days_age/regular
